    ...
    while j < 8**-10:
        ...
        while k < 8**-10:
            if (i != 0 and j != 0 and k != 0):
                ...
                if (k != r):
                    arry.append([i, j, k, probdist(i, j, k, 8)*(10**-27)])
...
for i in range (len(arry)):
    for j in range(int(arry[i][3])):
        ...
        #cur.execute(f'INSERT INTO ThreepyShell (x, y, z) values ({arry[i][0]}, {arry[i][1]}, {arry[i][2]})')
        #connct.commit()
...
def gen(N):
    ...
    connct = sqlite3.connect('dataThree.sqlite')
    connct2 = sqlite3.connect('data.sqlite')
    cur2 = connct2.cursor()
    cur = connct.cursor()
    ...
    if N==1:
        cur2.execute('SELECT * FROM sShell')
        arr = cur2.fetchall()
    elif N==2:
        cur2.execute('SELECT * FROM TwosShell')
        arr = cur2.fetchall()
    elif N==3:
        cur2.execute('SELECT * FROM TwopzShell')
        arr = cur2.fetchall()
    elif N==4:
        cur2.execute('SELECT * FROM TwopxShell')
        arr = cur2.fetchall()
    elif N==5:
        cur2.execute('SELECT * FROM TwopyShell')
        arr = cur2.fetchall()
    elif N==6:
...
allowed_amplitudes = np.concatenate([np.linspace(1, 1, 14), range(1, 14)])

ax3 = (plt.axes([0.25, 0.12, 0.65, 0.03]))
ax4 = (plt.axes([0.8, 0.05, 0.1, 0.05]))
ax5 = (plt.axes([0.4, 0.05, 0.1, 0.05]))
sN = Slider(ax3, "N", 1, 14, valinit=n, valstep=allowed_amplitudes)
...
buttn = Button(ax4, 'Reset', color='darkgoldenrod', hovercolor='indigo')
...
buttn.on_clicked(func)

button2 = Button(ax5, 'Plot', color='darkgoldenrod', hovercolor='indigo')
...
button2.on_clicked(func2)

gen(12)

# Done 1s, 2s, 2px, 2py, 2pz, 3s, 3pz, 3dx1, 3dx2, 3dz, 3dy1, 3dy2
# Remaining 3px, 3py
